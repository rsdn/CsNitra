...
- T4.4 пересобран (T4.4.1–T4.4.3): первая реализация — ручной `CommentStringTracker`
  (char-by-char скан комментариев/строковых литералов) — отклонена и откатана: она
  дублировала то, что уже есть в основном парсере. Замена: T4.4.1 — trivia-терминал
  препроцессора переиспользует `CSharpTerminals.Trivia()` (комментарии через основной
  парсер), T4.4.2 — проверка пропуска комментариев, T4.4.3 — строковые литералы
  пропускаются парсингом правила `Expression` основного парсера (пер-поточный
  `CSharpParser`, без глобальной лочки).
