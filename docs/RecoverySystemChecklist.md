...
- [x] Статус — завершено (сборка 0 ошибок; ParserTests 243 passed / 0 failed / 2 skipped)
...
- [x] Статус — завершено (сборка 0 ошибок; PatchRollbackTests 10/10, из них 3 новых; ParserTests 246 passed / 0 failed / 2 skipped)
...
- **Заметки:**
  - **Тесты (3):** (1) `Test_Hygiene_Removes_StartRule_AnyKind_At_CurrentStartPos` (scope b): через `SetMemo` — записи start-правила на currentStartPos (Partial + Success<EOF) и НЕ-start-правила (Success) на той же позиции → `Hygiene(...)` → записи start-правила удалены (ЛЮБОГО типа), запись НЕ-start-правила на месте. (2) `Test_Hygiene_Removes_Stale_Failure_At_Other_Positions` (scope c): stale Failure на pos=5 (≠ e=10, ≠ currentStartPos=0) + Success на pos=7 → `Hygiene(...)` → Failure удалена, Success на месте. (3) `Test_Rejected_S2_S3_Candidates_Leave_No_Memo_Phantoms` (e2e): полный `Parse` входа, где recovery реально применяет S2/S3-кандидатов → инвариант: в `Memo` НЕТ записей `ResultKind == Success && Node == null` (фантом `default(Result)`; `Node` — internal, проверка через `TryGetSuccess(out var node, out _)` — у фантома node==null) + все инъекции в `Injections` осмысленны (не `Length==0 && NodeKind==""`).
  - **E2E-вход и путь (проверено на практике, временная трассировка цикла):** `int f() { int x; } ### int g() { int y; }` + `MaxRecoveryAttemptsPerPosition = 10`. Цикл: iter0 e=19 — S0 + 4×S1 отклонены, **S2:Function:T1:23 принят** (resync, e 19→27, Skipped [19..23]); iter1 e=27 — S0 + 4×S1 отклонены, **S3:Function:Error принят** (panic, e 27→28, Skipped [27..28]); iter2 e=28 — S0 отклонён, S1:Function:) принят (e 28→31, Inserted); iter3 e=31 — S0, 4×S1, **S3:Expr:Error, S4:Expr:EOF, S5:Expr:Trailing применены и ОТКЛОНЕНЫ без прогресса** → recovery останавливается (ErrorInfo≠null, результат Success@19 < EOF). Финал: DIAG=3 (Skipped×2 + Inserted×1), INJ=3 (абсорберы [19..23)/[27..28) + вставка `)` — все осмысленны), MEMO=11.
  - **Почему бюджет 10 (отклонение от дефолта 3):** с `MaxRecoveryAttemptsPerPosition=3` цикл пробует только S0 + 2×S1 — S2/S3 (rank 2/3) генерируются, но НИКОГДА не применяются (проверено на практике на 8+ входах: в трассировке они не фигурируют). Бюджет 10 — минимально достаточный, чтобы S2 (6-я попытка на e=19) и S3 (6-я на e=31) реально прошли ApplyPatches/RollbackPatches.
  - **Результаты:** сборка `Nitra.sln` — 0 ошибок; `PatchRollbackTests` — 10/10 (7 старых + 3 новых); регрессия `Tests/ParserTests` — **246 passed / 0 failed / 2 skipped** (Total 248; 243 базовых + 3 новых; 2 — предсуществующие `[Ignore("WIP")]`).
  - **Прочее:** в `ParserTests.csproj` удалён leftover-include `<Compile Include="Recovery\ScratchE2ETests.cs" />` от прошлой сессии (файла не было → дубль SDK-глоба, NETSDK1022); csproj возвращён в коммитное состояние.

#### 1.3.2.4 Ленивый `Generate` после S0
- [x] Статус — завершено (сборка 0 ошибок; ParserTests 247 passed / 0 failed / 2 skipped)
...
- **Заметки:**
  - **Local function `TryCandidate(candidate) → bool`** (в теле recovery-цикла `Parse`): вынесено ВСЁ тело обработки одного кандидата — skip по `attempts.Contains(Id)`, `attempts.Add(Id)`, лимит `MaxRecoveryAttemptsPerPosition`, save `ErrorPos`+`_expected`, `ApplyPatches`, re-parse `ParseRule`, `RecoveryPointOf`, акцепт (Success@EOF или `e2 > e` → `result = next`, `recoveredThisIteration = true`), `RollbackPatches`+откат `ErrorPos`/`_expected` при отклонении, добавление `candidate.Diagnostics`. Возврат `true` = «стоп» (акцепт ИЛИ лимит — в обоих случаях `recoveredThisIteration` не тронут лимитом, т.е. цикл for завершится по `!recoveredThisIteration`), `false` = «следующий кандидат».
  - **Ленивая генерация:** `if (!TryCandidate(CandidateS0(e, snapshot, startRule))) { EngineGenerateCalls++; foreach (var candidate in RecoveryEngine.Generate(...)) if (TryCandidate(candidate)) break; }` — `Generate` вызывается только если S0 не дал прогресса. Хвостовой `return` при Success@EOF вынесен за блок кандидатов: `if (result.TryGetSuccess(out _, out var endAfter) && endAfter == input.Length) return result;` (эквивалентно прежнему `return next` внутри цикла — `result` к этому моменту равен принятому `next`).
  - **Поведение не изменилось (перестановка, не логика):** порядок проб (S0 → S1..S5) и акцепта тот же; лимит попыток и skip по `attempts` сохранены побайтово (проверено трассировкой: при достижении лимита `TryCandidate` возвращает `true`-стоп, как прежний `break`, и `attempts` пополняется тем же id).
  - **Счётчик `EngineGenerateCalls`:** `public int EngineGenerateCalls { get; private set; }` (паттерн `LastSnapshot`/`LastPartial`, public — нет InternalsVisibleTo). Сброс `= 0` в начале `Parse` (рядом с `_attempts.Clear()`); инкремент `++` в точке ленивого вызова `Generate`.
  - **Тест `IterativeRecoveryTests.Test_LazyGenerate_Counter` (1 метод, 2 ассерта):** (1) пропущенная `;` (`int f() { int x int y; }`) — S0 восстанавливает за одну итерацию → `EngineGenerateCalls == 0`; (2) пропущенная `(` (`int f ) { int x; }`, нет OftenMissed для `(`) — S0 не восстанавливает → `EngineGenerateCalls >= 1` (вставка S1). Оба входа → Success@EOF.
  - **Отклонение от ТЗ (второй ассерт):** ТЗ предлагало вход с несколькими ошибками (тест 6) → `>= 1`. На практике этот вход (`z = x $^ 5; ...`) восстанавливается ПОЛНОСТЬЮ S0 через recovery-правила грамматики (RecoveryOperator) по итерациям → `EngineGenerateCalls == 0` (ассерт `>= 1` упал). Заменён на вход с пропущенной `(` — там S0 действительно не даёт прогресса и Generate вызывается.
  - **Результаты:** сборка `Nitra.sln` — 0 ошибок; регрессия `Tests/ParserTests` — **247 passed / 0 failed / 2 skipped** (Total 249; 246 базовых + 1 новый; 2 — предсуществующие `[Ignore("WIP")]`). Все recovery-тесты (MiniC Error-правила, IterativeRecovery, AnchorResync, PatchRollback) без изменений в поведении.

### 1.3.3 Устранение предупреждений сборки (анализ + гуарды, без подавления)
- [x] Статус — завершено (сборка `Nitra.sln --no-incremental` 0 предупреждений / 0 ошибок; ParserTests 243 passed / 0 failed / 2 skipped; подавление не использовано)
- **Что:** все предсуществующие предупреждения в исходниках `ExtensibleParser` (те же файлы компилируются и в CppInteropGenerator/WorkflowGenerator — фикс в источнике чистит все три проекта): Parser.cs — CS8604 (possible null arg) + CS8602 (possible null deref); Result.cs — CS0618 ×2 (obsolete `Parser.Input`); FollowSetCalculator.cs — CS8600 + CS8620 (nullability). Точный уникальный список — свежая сборка `ExtensibleParser.csproj --no-incremental`.
- **Правила:**
  - **Подавление запрещено:** без `#pragma warning disable`, без `!` (null-forgiving), без `[SuppressMessage]`, без «слепых» nullable-аннотаций, скрывающих проблему без анализа.
  - Каждое предупреждение — анализ: (a) значение гарантированно non-null по построению → гуард `x.AssertIsNonNull()` (существующий паттерн `Regex/Regex/Guard.cs`, namespace `ExtensibleParser`; уже используется в Parser.cs:347, JsonParser, DotParser и др.); (b) null реально возможен → обработать (ветка/throw с сообщением); (c) CS0618 — разобрать использование `Parser.Input` в Result.cs: заменить на не-obsolete-механизм либо оставить с обоснованным комментарием (debug-only); (d) CS8600/CS8620 — корректные nullable-аннотации после анализа.
  - Поведение не меняется: гуарды бросают только там, где раньше было неопределённое поведение (null-deref).
- **Цель:** `dotnet build Nitra.sln --no-incremental` → **0 предупреждений, 0 ошибок**; ParserTests — все зелёные (243/0/2).
- **Тесты:** регрессия (полный прогон ParserTests).
- **Заметки:**
  - **Исходный список (свежая сборка `ExtensibleParser.csproj --no-incremental`) — 14 уникальных** (меньше ожидаемых ~64): **CS8604 ×9** (Parser.cs: 690, 692, 794, 821, 869, 1031, 1118, 1154, 1192), **CS8602 ×1** (Parser.cs: 522), **CS0618 ×2** (Result.cs: 127, 129), **CS8600 ×1** (FollowSetCalculator.cs: 353), **CS8620 ×1** (FollowSetCalculator.cs: 374).
  - **CS0618 (Result.cs 127,129) — «другим не-obsolete путём»:** `Parser.Input` — `[ThreadStatic][Obsolete] public static string?` (debug-only: отображение разобранного кода в Node/Result без хранения input в каждом узле). Реорганизация в `Parser.cs`: `[ThreadStatic] private static string? _debugInput` + `[Obsolete] public static string? Input => _debugInput` (getter-only — публичный контракт для внешних сохранён) + `internal static string? DebugInput => _debugInput` (внутренний debug-путь без `[Obsolete]`). Все внутренние debug-использования переключены на `Parser.DebugInput`: `Result.ToString`/`Result.DebugView`, `Node.Debug`/`Node.DebugContent`/`Node.DebugView` (SyntaxTree.cs), запись в `Parse` → `_debugInput = input`. Убраны ВСЕ `#pragma warning disable CS0618` (были в Result.cs ToString, SyntaxTree.cs, Parser.cs Parse). Поведение идентично (тот же thread-static; внешних потребителей `Parser.Input` в репо нет).
  - **CS8602 (Parser.cs 522) + CS8604 ×9 — единый паттерн:** `node`/`firstNode`/`sepNode`/`elemNode` — `out` из `TryGetSuccess`/`TryGetPartial` (`[MaybeNullWhen(false)]`), после которого guard (`return`/`break`/всегда-возвращающийся блок `if (!gotSuccess && !gotPartial)`) гарантирует non-null, но компилятор не трассует корреляцию через bool `gotSuccess`/`gotPartial`. Фикс: `x = x.AssertIsNonNull();` сразу после guard (паттерн `Guard.AssertIsNonNull` — бросает только в недостижимом случае). 10 мест: 522 (`node.Kind` в Log), 690/692 (Seq-элемент), 794 (OneOrMany first), 821 (OneOrMany loop), 869 (ZeroOrMany loop), 1031 (Seq loop), 1118 (SeparatedList first), 1154 (Separator), 1192 (SeparatedList elem).
  - **CS8600 (FollowSetCalculator.cs 353):** `Rule loopBody = loop switch { … _ => null }` — switch возвращает null → `Rule? loopBody` (значение реально nullable; дальше `if (loopBody is null) return;` сужает до non-null).
  - **CS8620 (FollowSetCalculator.cs 374):** `_followSets.GetValueOrDefault(refName, null)` — `null`-дефолт расширяет `TValue` до `HashSet<Terminal>?`, несовместимо с `Dictionary<string, HashSet<Terminal>>`. Фикс: `if (!_followSets.TryGetValue(refName, out var followSet)) followSet = new HashSet<Terminal>(…);` (эквивалентно по поведению; `TryGetValue` уже используется в файле, строка 68).
...
- [x] Статус — завершено (сборка 0 ошибок/0 предупреждений; RecoveryRuleTests 4/4 + S0IntegrationTests 4/4; ParserTests 259 passed / 0 failed / 2 skipped). Часть А (RecoveryRule + развёртка + Options в кадр) и Часть Б (удаление хака 0.3B → замена аннотацией) выполнены; хак НЕ восстановлен.
- **Что:** развёртка (поведение = `Inner`, только чтение аннотаций, без `RecoveryOptions`-полных — они в Фазе 2); S0-кандидат в единой детерминированной сортировке. + удаление хака 0.3B (Parser.cs:568-575).
- **Файлы:** `Rules.cs`, `Parser.cs`, `Tests/ParserTests/Recovery/RecoveryRuleTests.cs`, `Tests/ParserTests/Recovery/S0IntegrationTests.cs`, `Tests/ParserTests/MiniC/MiniCTests.cs`, `Tests/ParserTests/NitraConstruction/NitraSeparatedList/SeparatedListTests.cs`, `Tests/ParserTests/ParserTests.csproj`
- **Тесты:** `S0IntegrationTests`: MiniC-подобная грамматика с Error-правилами (RecoveryPrefix); `RecoveryRuleTests.ParsesLikeInner` (+ Options в кадр, TDOPP, инлайнинг).
- **Заметки:**
  - **RecoveryRule (создан, `Rules.cs` рядом с `OftenMissed`):** `public sealed record RecoveryRule(Rule Inner, Recovery.RecoveryOptions? Options = null) : Rule(Inner.Kind)`. `InlineReferences` → `new RecoveryRule(Inner.InlineReferences(...), Options)`; `GetSubRules<T>` → `this` (если `is T`) + делегирование `Inner`; `ToString` → `Inner.ToString()`. `RecoveryOptions` остался в `Recovery/ParseContext.cs` (переезд в `Rules.cs` — 2.1).
  - **Развёртка в `ParseAlternative`:** case `RecoveryRule r => ParseAlternative(r.Inner, startPos, input)` (рядом с `OftenMissed`). Поведение в точности = Inner: узел-обёртка в дереве НЕ создаётся (возвращается узел Inner как есть) — дерево/результат идентичны варианту без обёртки (корректный и некорректный входы).
  - **Options в StackFrame:** в `ParseRule` (push кадра альтернативы) `var altOptions = prefix is RecoveryRule rr ? rr.Options : null;` → `new StackFrame(ruleName, minPrecedence, new RuleFrameLocation(altIdx), null, altOptions)`. Вложенные Seq/Loop-кадры — как раньше (`null`). Проверено `Options_Land_In_AlternativeFrame`: кадр альтернативы (RuleFrameLocation) несёт именно Options обёртки (ReferenceEquals).
  - **BuildTdoppRules/TdoppRule:** RecoveryRule-префикс проходит BuildTdoppRules как обычная альтернатива (в `Prefix`/`RecoveryPrefix`), развёртка — в `ParseAlternative`. TDOPP-оператор с обёрнутым префиксом работает (`Tdopp_WrappedPrefix_ParsesLikeInner`: `Expr = RecoveryRule(Number) | Expr + Expr:100`, дерево = без обёртки); инлайнинг одно-префиксных правил не ломает (`WrappedSinglePrefixRule_ReferencedByRef`). **Ограничение:** RecoveryRule-обёртка САМОГО леворекурсивного оператора (`Seq([Ref(ruleName), …])`) не распознаётся как postfix (`alt is Seq` ложно) — на практике обёртывают не-операторные правила (Stmt/Block/Module, §3.9).
  - **Хак 0.3B (Parser.cs:568-575) — УДАЛЁН.** Гипотеза «S1 покрывает пропущенный операнд» **НЕ подтвердилась на практике**: (1) S1 вставляет терминал (First(Expr)=Number/Ident/…), а не ε-узел `Error` → другое дерево (и visitor падает на `int.Parse("")` для нулевой вставки); (2) S0 (re-parse) тоже не покрывает — без хака `ParseRule(Expr, e)` возвращает Failure, т.к. нулевой прогресс `ErrorEmpty`-префикса (ε-совпадение) отклоняется (`postNewPos == maxPos`, `bestResult == null`). **Упало 10 тестов** (4 MiniC + 6 SeparatedList): MiniC `FunctionCallWithOneComaOneArgs`/`FunctionCallWithTwoComasOneArgs`/`FunctionCallWithThreeComasWithTwoArgs`/`Err_MissingSecondExpression`; SeparatedList `RequiredCallWith1Args`/`RequiredCallWith2Args`/`RequiredCallWith3Args`/`ForbiddenCallWith1ArgsWitEndDelim`/`ForbiddenCallWith2ArgsWitEndDelimArgs`/`ForbiddenCallWith3ArgsWitEndDelim`.
  - **Замена (вариант ТЗ (б) — триггер по аннотации, а не по глобальному флагу):** в `ParseRule` нулевой прогресс принимается только если альтернатива аннотирована `RecoveryRule` (Recoverable, по умолчанию true): `else if (postNewPos == maxPos && bestResult == null && isRecoveryPos && prefix is RecoveryRule rc && (rc.Options is null || rc.Options.Recoverable))`. В грамматиках тестов ε-Error-правило `ErrorEmpty` обёрнуто в `RecoveryRule`: `MiniCTests` (1 место, standalone-префикс, НЕ тот что внутри `RecoveryEmptyOperator`), `SeparatedListTests` (3 места: ExpOptional/ExpRequired/ExpForbidden). Семантика: ε-совпадение Error-правила (пропущенный операнд/аргумент) принимается только для правил, явно помеченных автором как recoverable.
  - **Новые тесты:** `RecoveryRuleTests` (4): `ParsesLikeInner` (дерево/результат = Inner, с/без Options, корректный+некорректный вход), `Options_Land_In_AlternativeFrame`, `Tdopp_WrappedPrefix_ParsesLikeInner`, `WrappedSinglePrefixRule_ReferencedByRef`. `S0IntegrationTests` (4, MiniC-подобная грамматика с `RecoveryRule(ErrorEmpty)` в RecoveryPrefix): `MissingSemicolon_RecoveredByS0` (S0/OftenMissed → Success@EOF), `MissingParen_RecoveredByS1_HasInsertedDiagnostic` (S1-вставка `(` → `RecoveryDiagnostics` содержит Inserted), `MissingOperand_RecoveredByRecoveryRule` (`z = x + ;` → Success@EOF через ε-совпадение), `ValidInput_NoDiagnostics` (I6: корректный код → diag пуст).
  - **Фикс csproj (вне пункта, но блокировал сборку):** в `ParserTests.csproj` были leftover-`<Compile Include="Recovery\RecoveryRuleTests.cs" />` и `<Compile Include="Recovery\S0IntegrationTests.cs" />` от прошлой сессии → `NETSDK1022` (дубль SDK-глоба). Оба явных include удалены (файлы подхватываются глобом).
  - **Результаты:** сборка `Nitra.sln --no-incremental` — 0 ошибок / 0 предупреждений (новые/изменённые файлы чисты по диагностике); регрессия `Tests/ParserTests` — **259 passed / 0 failed / 2 skipped** (251 базовых + 4 RecoveryRuleTests + 4 S0IntegrationTests; 2 — предсуществующие `[Ignore("WIP")]`); `RegexTests` 9/9, `WiWorkflowTests` 1/1. Поведение на корректных входах не изменилось (I6: `ValidInput_NoDiagnostics` + все существующие MiniC/JSON/DOT/CsNitra зелёны).
