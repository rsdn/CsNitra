...
- [~] 1.3.6 Acceptance tests — D1.1 (120 errors → `Success@EOF`, all reported — done in 1.3.2); D1.2 (3+ errors, one unsavable → all reported + EOF + ≥1 `Unrecovered`); D1.3 (strict region → `Success@EOF` + `Unrecovered`, per C1 — NOT `FatalError`). Acceptance: all pass.
