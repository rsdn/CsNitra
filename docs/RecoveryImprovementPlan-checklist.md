...
- [~] 1.3.1 S6 node shape — fix `RecoveryEngine.GenerateS6` memo-patch node so downstream visitors handle it (not a `SeqNode` with the start-rule NAME as `Kind`). Prefix stays reachable by consumers; absorber (`IsAbsorber`) stays in the tree (I4: covers `[0,EOF)`); result `Success@EOF`. Acceptance: `ParserTests --filter "FullyQualifiedName~SeparatedListTests"` 0 failed; full `ParserTests` 0 failed. Do NOT modify the test visitor.
- [ ] 1.3.2 S6 guaranteed bottom — S6 exempt from the per-point budget (budget no longer stops S6); raise `MaxRecoveryIterations` so D1.1 (120 errors) finishes. Acceptance: D1.1 → `Success@EOF`; I6 (zero cost on correct code) preserved.
