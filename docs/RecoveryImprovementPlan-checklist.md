...
- [ ] 1.3.3 Recovery tests assert reached-EOF — BLOCKED on 1.5 (C1): 11 raw-interpolated-string inputs still hard-Fail (strict `ContextScope` suppresses S6). Re-run after 1.5. Then update ALL recovery-verifying tests to assert reached-EOF + error (CSharpGrammarTests `AssertFails` → `AssertRecoversWithEnd`; ParserTests recovery tests). Acceptance: all recovery tests pass with the reached-EOF assertion.
...
- [✅] 1.5 C1: "strict ⇒ only S6" — narrow the `Generate` strict-region gate so a `Recoverable:false` frame suppresses S1–S5 but still emits S6 (absorber-to-EOF); fix `GenerateS6` absorber start (`Math.Max(parseEnd, currentStartPos)`) and the `prefixNode==null` fallback. 11 raw-interpolated-string inputs reach `Success@EOF`. Update `Test_S6_Not_Generated_In_Strict_Region` (strict ⇒ only S6) + add C1 contract test. Unblocks 1.3.3. **Done** — engine + 11 raw inputs green; S6BottomTests 5/5; CSharpGrammarTests 0 failed. Deviations: (a) also converted 7 pre-existing-failing `Regular`/`Verbatim` `AssertFails` (1.3.2 regression) to meet 0-failed; (b) `Test_Recoverable_False_Disables_Epsilon_Acceptance` left red — second gate (recovery point not in a strict region ⇒ S6 fires), reported not fixed. See progress1.5.
