...
### H (аудит): минимальность hygiene + отсутствие квадратичности

**Проблема.** B1 (2.1) ограничил `HygieneCore` до точечного удаления, но инвариант «re-parse после ApplyPatches не переразбирает префикс `[0, currentStartPos)` повторно» нигде не зафиксирован и не измерен. При N ошибках на файле возможна ситуация O(N × file_size). Дополнительно: `HygieneCore` делает `_memo.Keys.ToList()` и полный обход memo на каждом кандидате в каждой итерации — на больших файлах это может доминировать во времени recovery.

**Что проверить:**

1. **Минимальность очистки.** На D1.1 (100+ мелких ошибок) и D1.6 (ошибки каждые N строк) — сколько ключей memo удаляется в сумме за Parse относительно общего размера memo? Если `HygieneRemovals` сопоставим с `_memo.Count` — очистка не минимальна, надо сузить.
2. **Memo-hit rate на re-parse.** После ApplyPatches в `TryCandidate` re-parse должен идти на memo-hit'ах по префиксу `[currentStartPos, e)` и на miss'ах только в окрестности `e` и после. Если miss'ов много — префикс переразбирается. Метрика: счётчики `MemoHits`/`MemoMisses` на `Parser` (сейчас их нет — добавить в рамках H), отношение hit/miss на re-parse.
3. **Стоимость `HygieneCore`.** `_memo.Keys.ToList()` создаёт копию ключей; полный обход memo выполняется на каждом `ApplyPatches`. Замерить: суммарное время в `HygieneCore` за Parse на D1.1/D1.6, среднее число ключей в memo на момент вызова, число вызовов. Если обход доминирует — нужен индекс (например, `Dictionary<int, List<key>>` по pos, чтобы обходить только релевантные ключи).
4. **Квадратичность.** На D1.6 (10 ошибок на 50 функций) — время recovery должно расти линейно с числом ошибок. Проверить на 10/50/100/500 ошибок (синтетический корпус), замерить время.
5. **Инвариант после rollback.** `RollbackPatches` восстанавливает memo-записи кандидата. Но записи, добавленные в memo **в процессе** re-parse (Success по префиксу, Partial и т.п.), не удаляются — они переживают откат. Это by design (I2), но надо убедиться, что memo не растёт неограниченно за много итераций recovery.

**Приёмка H:**
- `MemoHits`/`MemoMisses` на `Parser` (сброс в `Parse`), видны в D2-отчёте.
- На D1.1 суммарное `HygieneRemovals` ≤ O(N_errors × константа), не O(memo_size).
- На D1.6 hit rate на re-parse ≥ 90% (префикс действительно memo-hit'ится).
- Время `HygieneCore` на D1.1/D1.6 замерено и попадает в отчёт (D2).
- Если hygiene cost доминирует — отдельная подзадача «HygieneCore indexing» (не оптимизировать в рамках H).

**Место.** Wave 6, вместе с D2-full. Связь: `B1` (2.1) — реализация, `H` — аудит и метрики.
...
| H | Hygiene quality audit (минимальность + стоимость + hit rate) | B1 follow-up, perf-риск | 6 | высокий (риск квадратичности) |
...
> **B1: hygiene может быть неминимальной или дорогой на больших файлах.** `HygieneCore` делает полный обход memo на каждом кандидате; при 1000 итераций × N кандидатов × M ключей в memo — потенциальная квадратичность. Митигация: пункт H (Wave 6) — метрики `HygieneRemovals`, `MemoHits`/`MemoMisses`, время в `HygieneCore`. Если cost доминирует — отдельная подзадача «HygieneCore indexing».
