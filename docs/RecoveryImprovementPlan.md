...
> **Level 1 `MaxSkip ×4` — расширение, а не сужение.** Обоснование: отключён S2 T1 (спекулятивный parse, дорого per-position), остался T2 (First-скан, дёшево). Расширение MaxSkip компенсирует потерю покрытия от T1 более широким дешёвым сканом. «Деградация» здесь = «менее тщательно per-position, шире по диапазону». Net — дешевле.
...
**Порядок Wave 4 (обоснование):** A3 → A4-5 → B4 → P (отложен). A3 разблокирует три
отложенных пункта (1.4 A5-4, 2.2 A2-param, S1/S2/S3S6-бюджеты) и даёт базу для A4-5/B4.
A4-5 убирает recurring долг (noRec-фикс понадобился дважды). B4 закрывает DoD-3.
P — preventive: разблокирует A4-4 п.1 (~3% перфа, замедление 30.84→31.76ms) и A5-8;
после A3 у P может быть форма `RecoveryProfile.Mode`; после A4-5 P проще (нет `#if RECOVERY`).
...
## Бэклог (после Wave 5b)

### A6. Парные скобки — аддитивная регистрация, дружелюбная к расширениям

Проблема: S3 хардкодит три пары `{ } ( ) [ ]` тремя счётчиками; S6 вложенности не имеет вовсе — останавливается на закрывающей вложенного блока (обнажено в 5b.5/R2). Для расширяемого языка хардкод неприемлем: автор DSL или новая версия C# добавляют свои пары, в т.ч. многосимвольные (`{|` `|}`, `<[` `]>`, `«` `»`).

**Форма.** `Parser.AddBracketPair(Terminal open, Terminal close)` — идемпотентный, никогда не бросает, дедуп по `TerminalComparer.Instance`. Внутреннее представление — `opener → set-of-closers` (одна открывающая может иметь несколько допустимых закрывающих: `(` и `)` от одного автора, `(` и `]` от другого — регистрируются оба). Публичное read-only — `Parser.BracketPairs`. Дефолт при пустом объявлении — `{ } ( ) [ ]`.

**Требование расширяемости.** Регистрация множественная и аддитивная: базовый язык и каждое расширение добавляют пары независимо. Порядок регистрации не влияет на результат. Повторная регистрация той же пары — молча идемпотентна.

**Алгоритм.** S3: три счётчика `curly/paren/bracket` → `Stack<closerSet>`, шаг скана — `TryMatch` по всем объявленным открывающим/закрывающим (многосимвольные пары обязательны: `<[` поглощается одним матчем). На открывающей — push; на закрывающей — если stack непуст и вершина содержит эту закрывающую — pop; иначе скан продолжается. Продвижение `s` — как trivia-прыжок (`s += k - 1; continue`). S6 — та же логика через общий хелпер. При неоднозначности на позиции (объявлены `<` и `<[`) — longest-match.

**Наблюдаемость.** Повторная регистрация пары может подсвечиваться в `GrammarDiagnostics` (A4-6) — observation-only, никогда не ошибка.

**Связь с A7.** A6 меняет алгоритм S3-скана; A7 — механическая замена `List<Terminal>` на `HashSet<Terminal>` в стоп-наборах. Делать отдельными подзадачами. Порядок — A7 первым. Не объединять: смена алгоритма + смена типа коллекции одновременно = регресс не локализуется.

**Эффективность — обязательная проверка.** A6 трогает горячий путь S3-скана: `TryMatch` на каждой позиции × число пар может быть как быстрее, так и медленнее текущего `switch (input[s-1])`. Замеры на D1.2 и D1.6 до/после. Если регрессия на чистом корпусе > 5% — рассмотреть precompute первых символов открывающих/закрывающих для дешёвого пре-фильтра перед `TryMatch`.

**Подводные камни (проверить явно).**
1. Longest-match: `<` и `<[` объявлены — `<[` матчится как `<[`, не как `<`. Тест обязателен.
2. Стек не растёт неограниченно на `((((((...` — верхний предел `maxSkip`; зафиксировать тестом.
3. Регистрация только открывающей или только закрывающей — не бросать, не предупреждать.
4. Порядок пар в `BracketPairs` — детерминированный (не `Dictionary` итерация), иначе стабильность сортировки `ordered` в S3 ломается.
5. Interaction с R2: `AddSuffixObligations` добавляет суффиксные First в стоп-набор; балансировка A6 фильтрует чужие закрывающие. Проверить, что обе работают вместе.
6. Перф-ассерт: `S3ScanPositions` на D1.6 не растёт > шума.

**Место.** После Wave 5b, после A7.

---

### A7. Стоп-наборы recovery — множества, а не списки

Проблема: `stopSet` в S3/S6 — `List<Terminal>` + `Contains(t, TerminalComparer.Instance)` — линейный поиск. R2 добавил ещё один `Contains` в `AddSuffixObligations`. На грамматике с десятками терминаторов и десятками суффиксных обязательств — O(n²) на каждую позицию скана (скан идёт по всему региону до `maxSkip`). Квадрат убивает производительность recovery.

**Форма.** Заменить `List<Terminal>` + `Contains(...)` на `HashSet<Terminal>(TerminalComparer.Instance)` во всех recovery-путях, где есть такой паттерн. Пройти grep'ом по `List<Terminal>` + `Contains(`. Кандидаты (проверить все): S3 (`GenerateS3`), S6 (`GenerateS6`), `AddSuffixObligations`, `GetTerminators`/`GetTerminatorsPerSite`, `BuildExpectedSet`, `GetSuffixObligationFirsts`. Где порядок терминаторов важен (S1 — ранжирование, S3 `ordered` — origIdx) — держать и `HashSet` (для дедупа), и `List` (для порядка); не ломать стабильность сортировки.

**Связь с A6.** A7 — механическая замена типа коллекции, без изменения поведения. Делать отдельными подзадачами, A7 первым. Не объединять.

**Эффективность — обязательная проверка.** На маленьких грамматиках (1–3 терминатора) эффект нулевой — ожидаемо. Реальный эффект — на C#-грамматике и D1.6. Замеры до/после обязательны: D1.6 (много ошибок, длинные регионы) — время recovery; полный ParserTests + CSharpGrammarTests + CsPreprocessorTests — регрессов нет.

**Подводные камни (проверить явно).**
1. `GetTerminators`/`GetTerminatorsPerSite` возвращают `Terminal[]` — публичная сигнатура не меняется.
2. `BuildExpectedSet` возвращает `List<Terminal>` — порядок для «expecting {...}» важен; сохранить.
3. `TerminalComparer.Instance` — `IEqualityComparer<Terminal>`; `HashSet<Terminal>(TerminalComparer.Instance)` работает без правок компаратора.
4. Не ломать детерминизм: где итерация по стоп-набору влияет на выбор кандидата — сохранить порядок.
5. Замер до A7 — baseline для сравнения с A6. Если снимать только после A6, эффект A7 не отделить.

**Место.** После Wave 5b, до A6.

---

### F3+. F3 — тестовый сценарий

К текущему описанию F3 (`ActiveAltIndex` возвращает `null`, когда `RuleFrame` не прямо под `SeqFrame`; `FindSeq`/`GetSuffixObligationFirsts` идут упрощённым путём и могут подмешать терминалы неактивной альтернативы) добавить конкретный вход: грамматика с двумя альтернативами правила, разные суффиксы; проверить, что R2 (S3/S6-стоп-набор) и 7.5.2 (`FatalError`/`Unrecovered` expected) не подмешивают терминалы неактивной альтернативы. **R2 повысил приоритет F3**: теперь F3 влияет не только на финальное сообщение, но и на стоп-набор S3/S6. Вынести как отдельную подзадачу Wave 7+ (не смешивать с A6/A7).
