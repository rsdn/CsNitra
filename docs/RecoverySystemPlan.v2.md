...
| **S0 (OftenMissed re-parse, инлайн, высший приоритет) перехватывает S2 (resync T1/T2, ранг 2)**, когда S0 может сделать прогресс (ε-принять OftenMissed-терминал) | В грамматиках, где ошибка чинится OftenMissed-терминалом (MiniC `;`/`}`), T2 (`CanStart`) **не exercised** — восстановление идёт через S0 (без «resync point»-диагностики). T2 полезен только когда ошибка НЕ чинится S0/S1 (не OftenMissed, не вставляемый терминал) | Штатно по дизайну (S0 — самый точный/дешёвый, ранг 0). Для E2E-тестов T1/T2 использовать входы, которые S0/S1 не чинят (миним. грамматика без OftenMissed). **ТРЕБУЕТ ОТДЕЛЬНОГО АНАЛИЗА (будущее):** не должен ли T2 (`CanStart`) иметь приоритет над S0 в отдельных случаях (напр. когда следующая конструкция — известный якорь/CanStart)? Дизайн-вопрос о качестве восстановления, открытый. См. чек-лист 3.1.2b. |
